...
SITE_IDENTIFIER = 'cinemey' #identifant (nom de votre fichier) remplacez les espaces et les . par _ AUCUN CARACTERE SPECIAL
SITE_NAME = 'Cinemey' #nom que xbmc affiche
SITE_DESC = 'films en streaming, streaming hd, streaming 720p, Films/séries, récent' #description courte de votre source

URL_MAIN = 'https://cinemey.com/' #url de votre source
...
MOVIE_NEWS = (URL_MAIN + 'les-films-en-streaming/', 'showMovies') #films (derniers ajouts = trie par date)
MOVIE_MOVIE = (URL_MAIN + 'accueil-1/' , 'showMovies') #films (load source)
...
def showMovies(sSearch = ''):
    ...
    sPattern = '<figure><img src=([^"]+)alt=.+?title="([^"]+)".+?<a href="([^"]+)"'
    ...
    if (aResult[0] == True):
        ...
        for aEntry in aResult[1]:
            ...
            sTitle = aEntry[1].replace('film en streaming','')
            sUrl2 = aEntry[2]
            sThumb = aEntry[0]
            #sLang = aEntry[3]
            #sQual = aEntry[4]
...
def showHosters(): #recherche et affiche les hotes
    ...
    oParser = cParser()
    sPattern = 'data-url="([^"]+)" href="#player".+?alt="([^"]+)"'
